...
def medidas(ytest, predictions, predictions_proba):
    ...
    acc_1.append(ac)
    data = {'F1': F1_1, "accuracy" :acc_1, 'roc_score':ROC_score }

    return pd.DataFrame(data, columns=["F1", 'roc_score', 'accuracy'])
...
def modelos(xtrain, ytrain, xtest, modelo, ytest):
    start=time.time()
    model = modelo
    mode_train=model.fit(xtrain, ytrain)
    ...
    # curva de ganancia
    skplt.metrics.plot_cumulative_gain(ytest, predictions_proba_RF_PCA)
    plt.show()
    return start, end, prediccion, prediccion_proba, total_time, model_train
